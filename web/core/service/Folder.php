...
class Folder
{
     ...
     * Recursively merges folders with the same name
     *
     * @var string
     */
    const MERGE = 'merge';
     ...
     * @param bool|int $mode Mode (CHMOD) to apply to created folder, false to ignore
     * @link https://book.cakephp.org/2.0/en/core-utility-libraries/file-folder.html#Folder
     */
    public function __construct(bool|string $path = false, bool $create = false, bool|int $mode = false)
    {
        if (empty($path)) {
            $path = TMP;
        }
     ...
     */
    public function cd(string $path): bool|string
    {
     ...
     * @param bool|array $exceptions Either an array or boolean true will not grab dot files
     ...
     */
    public function read($sort = self::SORT_NAME, bool|array $exceptions = false, bool $fullPath = false)
    {
        $dirs = $files = array();
     ...
     * @return array Files matching pattern
     */
    protected function _findRecursive($pattern, $sort = false): array
    {
     ...
     */
    public static function isWindowsPath($path): bool
    {
     ...
     */
    public static function correctSlashFor($path): string
    {
     ...
     */
    public static function slashTerm($path): string
    {
     ...
     * @param bool|int $mode Octal value, e.g. 0755.
     ...
     */
    public function chmod(string $path, bool|int $mode = false, bool $recursive = true, array $exceptions = array()): bool
    {
        if (!$mode) {
            $mode = $this->mode;
        }
        ...
        if ($recursive === false && is_dir($path)) {
            ...
            if (@chmod($path, intval($mode, 8))) {
                ...
                $this->_messages[] = "$path changed to $mode";
                return true;
            }

            $this->_errors[] = "$path NOT changed to $mode";
            return false;
        }
            ...
            foreach ($paths as $type) {
                foreach ($type as $fullpath) {
                    ...
                    if (@chmod($fullpath, intval($mode, 8))) {
                        //@codingStandardsIgnoreEnd
                        $this->_messages[] = "$fullpath changed to $mode";
                    } else {
                        $this->_errors[] = "$fullpath NOT changed to $mode";
                    }
                }
            }
     ...
     * @param bool|array $exceptions Either an array of files/folder to exclude
     ...
     */
    public function tree(string $path = null, bool|array $exceptions = false, string $type = null): mixed
    {
        if (!$path) {
            $path = $this->path;
        }
     ...
     */
    public function create(string $pathname, bool|int $mode = false): bool
    {
        ...
        if (is_dir($path)) {
            ...
            foreach ($iterator as $item) {
                ...
                if ($item->isFile() || $item->isLink()) {
                    ...
                    if (@unlink($filePath)) {
                        //@codingStandardsIgnoreEnd
                        $this->_messages[] = "$filePath removed";
                    } else {
                        $this->_errors[] =  "$filePath NOT removed";
                    }
                } elseif ($item->isDir() && !$item->isDot()) {
                    //@codingStandardsIgnoreStart
                    if (@rmdir($filePath)) {
            ...
            if (@rmdir($path)) {
                //@codingStandardsIgnoreEnd
                $this->_messages[] = "$path removed";
            } else {
                $this->_errors[] =  "$path NOT removed";
                return false;
            }
        }
        return true;
    ...
    public function copy(array|string $options): bool
    {
        ...
        if (!$this->cd($fromDir)) {
            $this->_errors[] = "$fromDir not found";
            return false;
        }
        ...
        if (!is_writable($toDir)) {
            $this->_errors[] = "$fromDir not writable";
            return false;
        }
                ...
                if (($options['scheme'] != Folder::SKIP || !is_dir($to)) && !in_array($item, $exceptions)) {
                    ...
                    if (is_file($from) && (!is_file($to) || $options['scheme'] != Folder::SKIP)) {
                        if (copy($from, $to)) {
                            ...
                            $this->_messages[] = "$from copied to $to";

                        } else {
                            $this->_errors[] = "$from NOT copied to $to";
                        }
                    }
                    ...
                    if (is_dir($from) && !file_exists($to)) {
                        ...
                        if (mkdir($to, $mode)) {
                            ...
                            chmod($to, $mode);
                            umask($old);
                            $this->_messages[] = $to . " created";
                            $options = array('to' => $to, 'from' => $from) + $options;
                            $this->copy($options);
                        } else {
                            $this->_errors[] = $to . " not created";
                        }
     ...
     * @return bool Success
     */
    public function move(array $options): bool
    {
        $to = null;
        if (is_string($options)) {
            $to = $options;
     ...
     */
    public function realpath(string $path): bool|string
    {
